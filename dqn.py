...
CHECKPOINT_PATH = os.path.dirname(os.path.realpath(__file__)) + '/checkpoint/dqn'
MEMORY_SIZE = 100
BATCH_SIZE = 32
GAMMA = 0.999
INIT_EPSILON = 1.0
...
class DQN():
    ...
    def forward_pass(self):
        ...
        self.W_fc1, self.b_fc1 = self._fc_variable([6, 6])
        
        self.W_fc2, self.b_fc2 = self._fc_variable([6, 3])
        self.W_fc3, self.b_fc3 = self._fc_variable([6, 1])

        h_fc = tf.nn.relu(tf.matmul(h_flat, self.W_fc1) + self.b_fc1)

        output = tf.nn.softmax(tf.matmul(h_fc, self.W_fc2) + self.b_fc2)
        return output
